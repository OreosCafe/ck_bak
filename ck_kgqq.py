...
class KGQQCheckIn:
    ...
    def sign(cookie):
        ...
        url_15 = "https://node.kg.qq.com/webapp/proxy?t_stReward%3Aobject=%7B%22uInteractiveType%22%3A0%2C%22uRewardType%22%3A0%2C%22uFlowerNum%22%3A10%7D&ns=proto_music_station&cmd=message.get_reward&mapExt=JTdCJTIyY21kTmFtZSUyMiUzQSUyMkdldFJld2FyZFJlcSUyMiUyQyUyMmZpbGUlMjIlM0ElMjJwcm90b19tdXNpY19zdGF0aW9uSmNlJTIyJTJDJTIyd25zRGlzcGF0Y2hlciUyMiUzQXRydWUlN0Q&t_uUid={0}&t_strUgcId=".format(
            t_uuid)
        try:
            old_proto_profile_response = requests.get(url=proto_profile_url,
                                                      headers=headers)
            old_num = old_proto_profile_response.json(
            )["data"]["profile.getProfile"]["uFlowerNum"]
            nickname = old_proto_profile_response.json(
            )["data"]["profile.getProfile"]["stPersonInfo"]["sKgNick"]
            for url in url_list:
                try:
            ...
            for g_tk_openkey in range(16):
                try:
                    ...
                    vct_music_cards = proto_music_station_resp.json(
                    )["data"]["message.batch_get_music_cards"]["vctMusicCards"]
                    ...
                    u_flower_num = vct_music_cards_list["stReward"][
                        "uFlowerNum"]
                    if u_flower_num > 10:
            ...
            try:
                ...
                if vip_status == 1:
                    ...
                    )["data"]["vip.get_vip_day_reward"]["strTips"]
                    u_cur_reward_num = viprequest.json(
                    )["data"]["vip.get_vip_day_reward"]["uCurRewardNum"]
            ...
            new_proto_profile_response = requests.get(url=proto_profile_url,
                                                      headers=headers)
            new_num = new_proto_profile_response.json(
            )["data"]["profile.getProfile"]["uFlowerNum"]
