...
class GLaDOSCheckIn(object):
    def __init__(self, glados_cookie_list):
        self.glados_cookie_list = glados_cookie_list
    ...
    def main(self):
        msg_all = ""
        for glados_cookie in self.glados_cookie_list:
            cookies = glados_cookie.get("glados_cookie")
            try:
                check_in_response = self.api_check_in(cookies)
...
if __name__ == '__main__':
    data = get_data()
    _glados_cookie_list = data.get("GLADOS_COOKIE_LIST", [])
    res = GLaDOSCheckIn(glados_cookie_list=_glados_cookie_list).main()
    print(res)
    send("GLaDOS", res)