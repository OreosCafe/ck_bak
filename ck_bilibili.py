...
class BiliBiliCheckIn(object):
    ...
    def get_nav(session):
        ...
        current_exp = ret.get("data", {}).get("level_info",
                                              {}).get("current_exp")
    ...
    @staticmethod
    def vip_privilege_receive(session,
                              bili_jct,
                              receive_type: int = 1) -> dict:
        """
        领取B站大会员权益
    ...
    @staticmethod
    def report_task(session,
                    bili_jct,
                    aid: int,
                    cid: int,
                    progres: int = 300) -> dict:
        """
        B站上报视频观看进度
        aid int 视频av号
        cid int 视频cid号
    ...
    def main(self):
        ...
        for check_item in self.check_items:
            ...
            if is_login:
                ...
                coins_av_count = reward_ret.get("data",
                                                {}).get("coins_av") // 10
                ...
                if coin_num > 0:
                    for aid in aid_list[::-1]:
                        ...
                        if ret["code"] == 0:
                            ...
                            print(
                                f'投币{aid.get("title")}失败，原因为{ret["message"]}')
                            continue
                ...
                today_exp += coins_av
                update_data = (28800 - new_current_exp) // (today_exp if
                                                            today_exp else 1)
                msg = (
                    f"帐号信息: {uname}\n漫画签到: {manhua_msg}\n直播签到: {live_msg}\n"
                    f"登陆任务: 今日已登陆\n观看视频: {report_msg}\n分享任务: {share_msg}\n投币任务: {coin_msg}\n"
