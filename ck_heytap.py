...
class Heytap:
    ...
    def get_cookie_data(self):
        try:
            app_param = re.findall('app_param=(.*?)}',
                                   self.HT_cookies)[0] + '}'
            app_param = json.loads(app_param)
            self.sa_device_id = app_param["sa_device_id"]
            self.brand = app_param["brand"]
            self.sa_distinct_id = re.findall('sa_distinct_id=(.*?);',
                                             self.HT_cookies)[0]
            self.source_type = re.findall('source_type=(.*?);',
                                          self.HT_cookies)[0]
    ...
    def get_user_info(self):
        ...
        response = self.session.get(
            'https://www.heytap.com/cn/oapi/users/web/member/info',
            headers=headers)
    ...
    def taskCenter(self):
        ...
        res1 = self.client.get(
            'https://store.oppo.com/cn/oapi/credits/web/credits/show',
            headers=headers)
        res1 = res1.json()
        return res1
                        ...
                        if res['meta']['code'] == 200:
                            ...
                            for skuinfo in res['details'][0]['infos']:
                                ...
                                if i > 10:
                                    break
                                time.sleep(5)
                            res2 = self.cashingCredits(data['marking'],
                                                       data['type'],
                                                       data['credits'])
                            if res2:
                                self.log += '【每日浏览商品】：' + '任务完成！积分领取+' + str(
                                    data['credits']) + '\n'
            ...
            if qd['completeStatus'] == 0:
                ...
                while count <= endcount:
                    self.client.get(
                        ...
                        headers=headers)
                    count += 1
                res2 = self.cashingCredits(qd['marking'], qd['type'],
                                           qd['credits'])
                if res2:
                    self.log += '【每日分享商品】：' + '任务完成！积分领取+' + str(
                        qd['credits']) + '\n'
    ...
    def lottery(self, datas, referer='', extra_draw_cookie=''):
        ...
        res = self.client.post('https://hd.oppo.com/platform/lottery',
                               data=datas,
                               headers=headers)
        res = res.json()
        return res
    ...
    def task_finish(self, aid, t_index):
        ...
        res = self.client.post('https://hd.oppo.com/task/finish',
                               data=datas,
                               headers=headers)
        res = res.json()
        return res
    ...
    def task_award(self, aid, t_index):
        ...
        datas = "aid=" + str(aid) + "&t_index=" + str(t_index)
        res = self.client.post('https://hd.oppo.com/task/award',
                               data=datas,
                               headers=headers)
        res = res.json()
        return res
    ...
    def zaoshui_task(self):
        try:
            ...
            if res['code'] == 200:
                ...
                i = 0
                for data in record:
                    self.log += data['everydayDate'] + "——" + data[
                        'applyClockInStatus'] + "——" + data['credits'] + '\n'
...
def main_handler(event, context):
    ...
    with open(os.path.join(os.path.dirname(__file__), "check.json"),
              "r",
              encoding="utf-8") as f:
